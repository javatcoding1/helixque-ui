...
export default function ChatPanel({
  ...
  const canSend = Boolean(socket && socket.connected && roomId && name && (sidRef.current || mySocketId));
  ...
  useEffect(() => {
    ...
    const onMessage = (message: ChatMessage) => {
      ...
      if (!isOpen) {
        toast.success(`Peer: ${message.text.length > 80 ? `${message.text.slice(0, 77)}...` : message.text}`, {
          duration: 3500,
    ...
    const onSystem = ({ text, ts }: { text: string; ts?: number }) => {
      ...
      setMessages((prev) => {
        ...
        const next = [
          ...prev,
          { text: normalised, from: "system", clientId: "system", ts: ts ?? Date.now(), kind: "system" as const },
        ];
        return next.length > MAX_BUFFER ? next.slice(-MAX_BUFFER) : next;
      });
    };
    ...
    const onHistory = ({ roomId: rid, messages: incoming }: { roomId: string; messages: ChatMessage[] }) => {
      if (rid !== roomId) return;
      if (!Array.isArray(incoming) || incoming.length === 0) return;
      setMessages((prev) => {
        const keyOf = (item: ChatMessage) => `${item.kind || "user"}|${item.ts}|${item.clientId}|${item.text}`;
  ...
  const handleAddEmoji = (emoji: EmojiClickData) => {
    ...
    typingDebounceRef.current = setTimeout(() => {
      socket.emit("chat:typing", { roomId, from: name, typing: Boolean(newMessage) });
    }, TYPING_DEBOUNCE_MS);
  };
  ...
  const handleTyping = (value: string) => {
    ...
    if (typingDebounceRef.current) clearTimeout(typingDebounceRef.current);
    typingDebounceRef.current = setTimeout(() => {
      socket.emit("chat:typing", { roomId, from: name, typing: Boolean(value) });
    }, TYPING_DEBOUNCE_MS);
  };
    ...
    <div className="flex h-full flex-col overflow-hidden rounded-l-2xl bg-neutral-950">
      <div ref={scrollerRef} className="flex-1 space-y-2 overflow-y-auto px-3 py-3">
        {messages.map((message, index) => {
          ...
          return (
            <div key={`${message.ts}-${index}`} className={`flex ${isSystem ? "justify-center" : mine ? "justify-end" : "justify-start"}`}>
              <div
                className={
                  isSystem
              ...
              >
                {isSystem ? (
                  ...
                  <>
                    {!mine && <div className="mb-1 text-[10px] text-white/60">Peer</div>}
        ...
        {peerTyping && <div className="text-xs italic text-white/60">{peerTyping}</div>}
      </div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <Input
              ...
              onClick={(event) => setCursorPosition(event.currentTarget.selectionStart || input.length)}
              onKeyUp={(event) => setCursorPosition(event.currentTarget.selectionStart || input.length)}
              onKeyDown={(event) => {
                if (event.key === "Enter") {
