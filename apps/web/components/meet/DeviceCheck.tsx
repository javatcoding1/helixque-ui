    ...
    <div className="flex min-h-screen items-center justify-center bg-neutral-950 px-6 py-8">
      <div className="mx-auto w-full max-w-5xl">
        <div className="mb-12 space-y-1 text-center">
          ...
          <p className="text-sm text-neutral-400">Check your camera and microphone before joining</p>
        </div>
            ...
            <div className="relative flex-1 overflow-hidden rounded-2xl border border-white/10 bg-black shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
              <div className="relative aspect-video w-full bg-black">
                ...
                <div className="absolute bottom-3 left-3 flex items-center gap-2">
                  <span className="rounded-md bg-black/60 px-2 py-1 text-xs text-white">{name || "You"}</span>
                  {!audioOn && (
                ...
                <TooltipTrigger asChild>
                  <Button
                    ...
                    className={`h-11 w-11 rounded-full ${
                      audioOn ? "bg-white/10 text-white hover:bg-white/20" : "bg-red-600 text-white hover:bg-red-500"
                    }`}
                  >
                    {audioOn ? <IconMicrophone className="h-5 w-5" /> : <IconMicrophoneOff className="h-5 w-5" />}
                  </Button>
                </TooltipTrigger>
                <TooltipContent>{audioOn ? "Turn off microphone" : "Turn on microphone"}</TooltipContent>
              </Tooltip>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    size="icon-lg"
                    className={`h-11 w-11 rounded-full ${
                      videoOn ? "bg-white/10 text-white hover:bg-white/20" : "bg-red-600 text-white hover:bg-red-500"
                    }`}
                  >
                    {videoOn ? <IconVideo className="h-5 w-5" /> : <IconVideoOff className="h-5 w-5" />}
                  </Button>
                </TooltipTrigger>
                <TooltipContent>{videoOn ? "Turn off camera" : "Turn on camera"}</TooltipContent>
              </Tooltip>
