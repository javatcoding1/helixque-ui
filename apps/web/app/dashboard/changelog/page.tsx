...
const CHANGELOG = [
  {
    ...
    changes: [
      { type: "New", description: "Launched AI Assistant with intelligent context awareness." },
      { type: "New", description: "Professional Connect marketplace for finding mentors." },
      { type: "New", description: "Integrated video calling for mentor sessions." },
      { type: "Improvement", description: "Enhanced sidebar navigation and mobile responsiveness." },
    ]
  },
  {
    version: "v2.0.5",
    ...
    changes: [
      ...
      { type: "Fix", description: "Fixed hydration issues on dashboard charts." },
      { type: "Improvement", description: "Updated color palette for better accessibility." },
    ]
  },
  {
    version: "v2.0.0",
    ...
    changes: [
      { type: "New", description: "Complete rebranding from 'Instant Connect' to 'Helixque'." },
      { type: "New", description: "Dark mode is now the default experience." },
      { type: "New", description: "Launched new landing page and marketing assets." },
    ]
  },
];
    ...
    <div className="flex flex-col h-full bg-background overflow-y-auto p-6">
      <div className="max-w-3xl mx-auto w-full space-y-12">
        <div className="text-center space-y-4">
           ...
           <p className="text-xl text-muted-foreground">Latest updates and improvements to Helixque.</p>
        </div>
             ...
             <div key={release.version} className="relative pl-8 md:pl-12">
                ...
                <div className="space-y-6">
                   <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                      ...
                      <span className="text-sm text-muted-foreground bg-muted/50 px-2 py-1 rounded">{release.date}</span>
                   </div>
                      ...
                      <ul className="space-y-4">
                        {release.changes.map((change, i) => (
                          <li key={i} className="flex gap-4 items-start">
                             <Badge variant="outline" className={`${getTypeColor(change.type)} shrink-0 mt-0.5`}>
                               {change.type}
                             </Badge>
