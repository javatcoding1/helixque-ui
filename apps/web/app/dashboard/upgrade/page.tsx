...
const PLANS = [
  ...
  },
  {
    ...
    period: "/month",
    description:
      "Perfect for active networkers looking for quality connections.",
    features: [
      "50 Skips per day",
      "Priority Matching",
      "Verified Badge",
    ...
    <div className="flex flex-col h-full overflow-y-auto bg-background">
      ...
      <div className="relative overflow-hidden border-b border-border bg-background/50 py-24 sm:py-32">
        ...
        <div className="relative max-w-6xl mx-auto px-6 text-center space-y-6">
          <Badge
            variant="secondary"
            className="px-4 py-1.5 text-sm rounded-full border border-primary/20 bg-primary/5 text-primary"
          >
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed">
            Get more skips, priority matching, and exclusive features to
                ...
                <CardHeader className="space-y-4 pb-8 pt-8">
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      ...
                      {plan.icon && (
                        <plan.icon className="h-6 w-6 text-primary absolute top-8 right-6 opacity-20" />
                      )}
                    </div>
                  </div>
                    ...
                    <ul className="space-y-4 text-sm mt-4">
                      {plan.features.map((feature, i) => (
                        <li key={i} className="flex items-start gap-3.5">
                          <div
                            className={`shrink-0 h-5 w-5 rounded-full flex items-center justify-center mt-0.5 ${plan.highlight ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
                          >
            ...
            <div className="bg-background/40 backdrop-blur-sm rounded-[14px] p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
              <div className="space-y-3 max-w-2xl">
                ...
                <p className="text-lg text-muted-foreground">
                  Looking to onboard your entire organization? We offer custom
                  SSO, dedicated support, and bulk pricing.
                </p>
              </div>
