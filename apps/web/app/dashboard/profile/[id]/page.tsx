...
export default function ProfileViewPage() {
  ...
  const [connectionStatus, setConnectionStatus] = useState<'NONE' | 'PENDING_SENT' | 'PENDING_RECEIVED' | 'FRIENDS'>('NONE');
  ...
  const handleConnectAction = () => {
       toast("Connections updating...", {
           description: "We are upgrading our social infrastructure. Connecting will be back soon!",
               ...
               <div className="flex flex-col md:flex-row items-start md:items-end gap-6 -mt-12">
                   ...
                   <div className="flex-1 space-y-2 pt-2 md:pt-0">
                       <div>
                           ...
                           <p className="text-muted-foreground font-medium">{user.headline || user.role || "Member"}</p>
                       </div>
                   ...
                   <div className="flex gap-2 mb-2 md:mb-0">
                       {isOwnProfile ? (
                           <Button variant="outline" onClick={() => router.push('/dashboard/edit-profile')}>
                               Edit Profile
                           </Button>
                       ) : (
                    ...
                    <CardContent>
                        <div className="flex flex-wrap gap-2">
                            {user.languages && user.languages.length > 0 ? (
                                ...
                                <span className="text-muted-foreground text-sm">No languages listed.</span>
                            )}
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        {user.preferences?.workingHours && (
                            <div className="space-y-1">
                                <span className="text-xs text-muted-foreground block">Working Hours</span>
                                <span className="text-sm font-medium">{user.preferences.workingHours}</span>
                            </div>
                        )}
                    </CardContent>
                </Card>
